...
class Main:
    ...
    def __init__(self):
        ...
        self.ExpertRate = 0.2
        self.Expert = np.zeros(self.n_workers, dtype=bool)  # To use expert or not (per action/episode/batch)
        self.ExpertDecayRate = 1000000
        self.ExpertDecay = 1
        self.ExpertUse = "episode"  # action, batch, episode
        self.ExpertActualUseCount = 0  # stores number of times the expert was actually used,starting with one just to avoid 0/0
        self.ExpertUseCount = 1
        ...
        self.gamma = 0.99
        self.lmbda = 0.95
        self.ExpertThresh = 0.5 * 1 / self.NumOfActions
        ...
        self.Expert_Path = "../Experts/Expert{}.pt".format(self.NumOfWalls)
        ...
        self.PPO = PPO(self.state_dim, self.MapDimensions, self.action_dim, self.lr_actor, self.lr_critic, self.gamma,
                       ...
                       self.NumOfAgents, self.EncoderLocation, self.EncoderDims, self.Expert_Path, self.ExpertRate)
        ...
        self.ConsoleOutput = ["\ncurrent logging run number for " + "MultiAgentLocal" + " : {}".format(run_num),
                              ...
                              "Expert Rate : {}".format(self.ExpertRate),
                              "Expert Decay and Decay Rate: {} and {}".format(self.ExpertDecay, self.ExpertDecayRate),
                              "---------------------------------------------------------------------------------------",
    ...
    def sample(self) -> (Dict[str, np.ndarray], List):
        ...
        if self.ExpertUse == "batch":
            ExpertProb = np.random.rand()
            self.Expert = np.ones(self.n_workers, dtype=bool) * (ExpertProb <= self.ExpertRate)

        for t in range(self.worker_steps):
            if self.ExpertUse == "action":
                ExpertProb = np.random.rand(self.n_workers)
                self.Expert = ExpertProb <= self.ExpertRate
            elif self.ExpertUse == "episode" and np.sum(WorkersNewEpisode) > 0:
                ExpertProb = np.random.rand(np.sum(WorkersNewEpisode))
                self.Expert[WorkersNewEpisode] = (ExpertProb <= self.ExpertRate)

            actions, ExpertUseCount = self.PPO.select_action(self.obs, self.Walls, self.InvalidActions, self.Learn, t,
                                                        self.done, self.Expert, self.ExpertThresh)
            if np.sum(self.Expert) > 0:
                self.ExpertActualUseCount += ExpertUseCount
                self.ExpertUseCount += np.sum(self.Expert)
        ...
        for update in range(self.StartingBatch, self.updates):
            ...
            if (update+1) % self.print_freq == 0 and print_running_batches > 0:
                ...
                outputt = "\nEpisode: %d, TimeStep: %d, AverageReward: %.2f, AverageSteps: %.2f, AverageCost: %.2f, " \
                          "AverageSuccess: %.2f, Elapsed time: %s, Expert Actual Rate: %.2f" % (self.Episode, CurrStep,
                            AverageBatchReward, AverageBatchSteps, AverageBatchCost, AverageBatchSuccess,
                            datetime.now().replace(microsecond=0) - self.start_time, (self.ExpertRate > 0) *
                            self.ExpertActualUseCount / self.ExpertUseCount)
            ...
            if (update+1) % self.ExpertDecayRate == 0:
                self.ExpertRate = self.ExpertRate * self.ExpertDecay
    ...
    def test(self):
        ...
        Expert_Testing = 0  # do not use expert for testing

        for t in range(self.worker_steps * self.NumOfTestBatches):

            actions, _ = PPO_test.select_action(obs, Walls, InvalidActions, Learn, t, done, Expert_Testing, self.ExpertThresh)
