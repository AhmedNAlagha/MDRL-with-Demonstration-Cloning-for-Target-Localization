...
class Environment:
    ...
    def InitialState(self):
        ...
        if self.NumOfWalls > 0:
            self.RewardsMap = self.ComputeRewardsMap(self.TargetLocationsGrid[0])
        else:
        ...
        for i in range(len(self.TargetLocations)):
            self.prev_shortest_distance = self.FindDistanceToClosest(self.AgentsPrevLocationsGrid)
            self.Current_shortest_distance = self.prev_shortest_distance
    ...
    def step(self, actions):
        ...
        for i in range(len(self.TargetLocations)):
            self.prev_shortest_distance = self.Current_shortest_distance
            self.Current_shortest_distance = self.FindDistanceToClosest(self.AgentsLocationsGrid)
            if self.Current_shortest_distance < self.prev_shortest_distance:
                reward += 1
            else:
                reward += -1
    ...
    def FindDistanceToClosest(self, Locations):
        distance = float('inf')
        for location in Locations:
            checkDistance = self.RewardsMap[location[0]][location[1]]
